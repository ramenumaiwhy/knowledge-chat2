...
class ChibaStyleValidator {
  ...
  checkStructuralSimilarity(text) {
    ...
    // チバさんの平均段落長（349文字）との類似度
    const lengthSimilarity = 1 - Math.abs(avgLength - 349) / 349;
    score += Math.max(0, lengthSimilarity) * 30;
    totalChecks += 30;

    // 改行パターン
    const lineBreaks = (text.match(/\n/g) || []).length;
    const expectedBreaks = text.length / 50; // 約50文字ごとに改行
    const breakSimilarity = 1 - Math.abs(lineBreaks - expectedBreaks) / expectedBreaks;
